...
RED HAT's central rule is that **players cannot know their own helmet color (role)**.  
...
This means:
- Role assignment runs on the server only. Clients never receive their own helmet color.
- Gun resolution (real bullet vs. self-detonation) runs on the server only. The client only sends a "fire requested" input.
- The detonation timer, task completion state, and extraction logic are all server-authoritative.
